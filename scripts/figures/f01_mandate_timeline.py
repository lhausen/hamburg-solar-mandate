...
first-round candidates and pointed at mandate_index_monthly.csv — the
actual mandate_commercial / mandate_private / mandate_score (0-2) index that
feeds Panel C and every DiD/SCM script downstream. Ambiguous/narrow-scope
...
construction). Dates read directly from mandate_index_monthly.csv (first
...
after mandate_index_monthly.csv was built, so the panel has no repeal
...
SRC  = BASE / "data" / "panels" / "mandate_index_monthly.csv"
OUT  = BASE / "output" / "figures" / "f01_mandate_timeline.png"
...
# Repeals not reflected in mandate_index_monthly.csv (found 2026-08-09, see
...
fig.text(0.5, 0.005,
         "Source: Auswertung/Panels/mandate_index_monthly.csv (Skripte/build_mandate_index.py) — the index "
         "used in Panel C and every DiD/SCM regression.",
