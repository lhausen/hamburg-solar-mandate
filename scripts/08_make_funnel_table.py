...
Every number is computed fresh:
  Block 1 (rows 1-5) — the four-filter cascade, applied step by step directly
    ...
    ground-mounted/other non-roof structures -> restrict
...
RAW_MASTR   = BASE / "data" / "raw" / "mastr_solar_deutschland.csv"
SCM_ANNUAL  = (BASE / "output" /
               "scm" /
               "Data" / "scm_panel_annual.csv")
DID_BASE    = (BASE / "output" / "did" /
               "05__per_km2__privat__start2015" / "base" / "Data" / "panel_2015_base.csv")

TEX_OUT = BASE / "output" / "tables" / "data_funnel_table.tex"
CSV_OUT = BASE / "output" / "reports" / "04_data_funnel.csv"
...
n_status = n_balkon = n_nonroof = n_window = 0
...
for chunk in pd.read_csv(
    ...
    # 2026-08-27: capacity filter removed (user decision)
    c3 = c2[c2["Inbetriebnahmedatum"].between("2015-01-01", "2025-12-31")]
    n_window += len(c3)
