...
Most of the analysis runs from the panels included in this repository. Unpack
Panel A first:
...
python3 Skripte/Descriptive/run_all.py          # descriptive figures, summary table
```

The remaining scripts read the raw MaStR export directly and therefore require
the download described above:
...
The first five rebuild the panels from scratch and reproduce the files shipped
here. The last two count records at each filter step, which can only be done on
the unfiltered export.

All outputs generated by the first group are included under `Auswertung/`, so
the results can be checked without running anything.
